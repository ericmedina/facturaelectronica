			  	...
			  	@foreach($movimientos as $movimiento)
				   	...
				   	@if($movimiento->movimiento == "VENTA")
				        <td data-label="Fecha">{{ $movimiento->venta->fecha }}</td>
					   	...
					   	<td data-label="Saldo">${{ $movimiento->saldo }}</td>
				   	...
				   	@elseif($movimiento->movimiento == "COMPRA")
				        <td data-label="Fecha">{{ $movimiento->compra->fecha }}</td>
					   	...
					   	<td data-label="Cliente">{{ $movimiento->compra->cliente->nombre }}</td>
				   	...
				   	@elseif($movimiento->movimiento == "COBRO CUENTA CORRIENTE")
				        <td data-label="Fecha">{{ $movimiento->resumen_cuenta->fecha }}</td>
					   	...
					   	<td data-label="Saldo">${{ $movimiento->saldo }}</td>
					   	<td><a href="{{url('facturas/'. 'RC'.str_pad($movimiento->resumen_cuenta->id,8,"0",STR_PAD_LEFT) )}}" target="_blank" style="margin-top:2.5px;" class="btn btn-warning btn-tabla btn-editar float-left">PDF</a></td>
